...
const ProductImages = ({ images }: Props) => {
  ...
  const SwiperNavigation = () => {
    ...
    return (
      <div className="swiper-navigation">
        <button onClick={() => swiper.slidePrev()} className="swiper-button-prev">Prev</button>
  ...
  const SwiperPagination = () => {
    ...
    return (
      <div className={`swiper-pagination ${isActive ? 'active' : ''}`} />
    );
  };
      ...
      >
        {images.map((image, index) => (
          <SwiperSlide key={index}>
            <img src={image} alt={`Imagem do produto ${index}`} className="w-full" />
            <SwiperPagination />
          </SwiperSlide>
        ))}
        <SwiperNavigation />
