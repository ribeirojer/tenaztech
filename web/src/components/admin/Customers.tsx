...
const toggleDropdown  = (id: any) => {
  ...
  return (
    ...
    <div className="flex items-center">
      ...
      <button className="ml-auto px-4 py-2 bg-blue-500 text-white rounded-md text-sm">Add customer</button>
    </div>
        ...
        <tbody>
          <tr v-for="customer in customers" key="customer.id">
            <td className="py-2 px-3">
              <img className="w-10 h-10 rounded-full" src="customer.image || '/placeholder-user.jpg'" alt="customer.name" />
            </td>
            <td className="py-2 px-3 font-medium">{ customers.name }</td>
            <td className="py-2 px-3 hidden md:table-cell">{ customers.email }</td>
            <td className="py-2 px-3 hidden md:table-cell">{ customers.totalOrders }</td>
            <td className="py-2 px-3 hidden md:table-cell">{ customers.lastOrder }</td>
            <td className="py-2 px-3 text-right">
              <div className="relative">
                <button onClick={()=>toggleDropdown(customers.id)} className="px-2 py-1 text-gray-500 hover:text-gray-900 focus:outline-none">
                ...
                <div v-if="dropdownVisible === customer.id" className="absolute right-0 z-10 bg-white shadow-lg rounded-lg mt-1">
                  <a href="#" className="block px-4 py-2 text-gray-700 hover:bg-gray-100">View</a>
