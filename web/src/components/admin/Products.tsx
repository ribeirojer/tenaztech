...
const Products = (props: Props) => {
  ...
  return (
    ...
    <div className="flex items-center">
      ...
      <button className="ml-auto px-4 py-2 bg-blue-500 text-white rounded-md text-sm">Add product</button>
    </div>
        ...
        <tbody>
          ...
          <tr v-for=" in " key="product.id">
            <td className="py-2 px-3">
              <img src="product.image || '/motivation.svg'" alt="product.name" className="aspect-square rounded-md object-cover" width="64" height="64" />
            </td>
            <td className="py-2 px-3 font-medium">{ product.name }</td>
            <td className="py-2 px-3 hidden md:table-cell">{ product.status }</td>
            <td className="py-2 px-3">{ product.inventory }</td>
            <td className="py-2 px-3">{ product.vendor }</td>
            <td className="py-2 px-3 text-right">
              <div className="relative">
                <button onClick={()=>toggleDropdown(product.id)} className="px-2 py-1 text-gray-500 hover:text-gray-900 focus:outline-none">
                ...
                <div v-if="dropdownVisible === product.id" className="absolute right-0 z-10 bg-white shadow-lg rounded-lg mt-1">
