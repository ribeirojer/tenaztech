      ...
      <div class="flex flex-wrap -mx-4 lg:justify-between">
        ...
        <div class="w-full px-4 lg:w-1/2 xl:w-6/12">
          <div class="mb-12 max-w-[570px] lg:mb-0">
            <span class="block mb-4 text-base font-semibold text-purple-500">Contact Us</span>
            <h2 class="text-dark dark:text-white mb-6 text-[32px] font-bold uppercase sm:text-[40px] lg:text-[36px] xl:text-[40px]">GET IN TOUCH WITH US</h2>
            <p class="text-base leading-relaxed text-body-color dark:text-dark-6 mb-9">Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eius tempor incididunt ut labore et dolore magna aliqua. Ut enim adiqua minim veniam quis nostrud exercitation ullamco</p>
            <div class="mb-8 flex w-full max-w-[370px]">
              <div class="bg-purple-100 text-purple-500 mr-6 flex h-[60px] w-full max-w-[60px] items-center justify-center overflow-hidden rounded sm:h-[70px] sm:max-w-[70px]">
  ...
  <div class="w-full px-4 lg:w-1/2 xl:w-5/12">
    <section class="relative p-8 bg-white rounded-lg shadow-lg dark:bg-dark-2 sm:p-12">
      <form @submit.prevent="submitForm">
        <div class="mb-6">
          <Input :id="'name'" :label="'Nome: '" type="text" placeholder="Seu Nome" v-model="name" class="border-stroke dark:border-dark-3 dark:text-dark-6 dark:bg-dark text-body-color focus:border-purple-500 w-full rounded border py-3 px-[14px] text-base outline-none" ref="nameInput"/>
          <span v-if="nameError" class="text-red-500">{{ nameError }}</span>
        </div>
        <div class="mb-6">
          <Input :id="'email'" :label="'E-mail: '" type="email" placeholder="Seu Email" v-model="email" class="border-stroke dark:border-dark-3 dark:text-dark-6 dark:bg-dark text-body-color focus:border-purple-500 w-full rounded border py-3 px-[14px] text-base outline-none" ref="emailInput"/>
          <span v-if="emailError" class="text-red-500">{{ emailError }}</span>
        </div>
        <div class="mb-6">
          <Input :id="'phone'" :label="'Numero de telefone: '" type="tel" placeholder="Seu Telefone" v-model="phone" pattern="[0-9]{10,14}" class="border-stroke dark:border-dark-3 dark:text-dark-6 dark:bg-dark text-body-color focus:border-purple-500 w-full rounded border py-3 px-[14px] text-base outline-none" ref="phoneInput"/>
