...
const asyncPaymentProcessor = new AsyncPaymentProcessor(orderRepository, paymentService, emailService);

setInterval(async () => {
    try {
        const pendingOrders = await orderRepository.findByStatus(new OrderStatus('pending'));
        ...
        for (const order of pendingOrders) {
            try {
                ...
                console.error(`Error processing order ${order.getId().getValue()}:`, error);
            }
        }
    } catch (error) {
