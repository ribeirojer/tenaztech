...
describe("Cart Routes", () => {
    ...
    describe("POST /api/carts/:userId/items", () => {
        it("Should add an item to the cart", async () => {
            ...
            const response = await request(app).post(`/api/carts/${userId}/items`).send(newItem);
        ...
        it("Should return 400 if the request body is invalid", async () => {
            ...
            const response = await request(app).post(`/api/carts/${userId}/items`).send(invalidItem);
    ...
    describe("PUT /api/carts/:userId/items/:itemId", () => {
        it("Should update an item in the cart", async () => {
            ...
            const response = await request(app).put(`/api/carts/${userId}/items/${itemId}`).send(updatedItem);
            expect(response.status).toBe(200);
            expect(response.body).toHaveProperty('items');
        });
    ...
    describe("DELETE /api/carts/:userId/items/:itemId", () => {
        it("Should remove an item from the cart", async () => {
            const userId = "validUserId";
            const itemId = "validItemId";
            const response = await request(app).delete(`/api/carts/${userId}/items/${itemId}`);
            expect(response.status).toBe(200);
            expect(response.body).toHaveProperty('items');
        });
    ...
    describe("POST /api/carts/:userId/checkout", () => {
        it("Should checkout the cart", async () => {
            ...
            const response = await request(app).post(`/api/carts/${userId}/checkout`).send(checkoutData);
            expect(response.status).toBe(201);
        ...
        it("Should return 400 if the request body is invalid", async () => {
            ...
            const response = await request(app).post(`/api/carts/${userId}/checkout`).send(invalidCheckoutData);
            expect(response.status).toBe(400);
            expect(response.body).toHaveProperty('error');
        });
    });
