describe("Product Review Routes", () => {
    describe("POST /api/products/:productId/reviews", () => {
        it("Should add a review to a product", async () => {
            ...
            const response = await request(app).post(`/api/products/${productId}/reviews`).send(newReview);
        ...
        it("Should return 400 if the request body is invalid", async () => {
            ...
            const response = await request(app).post(`/api/products/${productId}/reviews`).send(invalidReview);
