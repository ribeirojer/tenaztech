...
describe("Wishlist Routes", () => {
    ...
    describe("POST /api/wishlists/items", () => {
        it("Should add an item to the wishlist", async () => {
            ...
            const response = await request(app).post("/api/wishlists/items").send(newItem);
        ...
        it("Should return 400 if the request body is invalid", async () => {
            ...
            const response = await request(app).post("/api/wishlists/items").send(invalidItem);
    ...
    describe("DELETE /api/wishlists/items/:itemId", () => {
        it("Should remove an item from the wishlist", async () => {
            ...
            const response = await request(app).delete(`/api/wishlists/items/${itemId}`);
            expect(response.status).toBe(200);
        ...
        it("Should return 404 if the item is not found", async () => {
            ...
            expect(response.body).toHaveProperty('error', 'Item not found in wishlist');
        });
    });
    ...
    describe("PUT /api/wishlists/items/:itemId", () => {
        it("Should update an item in the wishlist", async () => {
            ...
            const response = await request(app).put(`/api/wishlists/items/${itemId}`).send(updatedItem);
            expect(response.status).toBe(200);
