...
export class InventoryController {
  ...
  getStockLevel = async ({ params, response }: { params: { productId: string }; response: any }) => {
    try {
      logger.info(`Fetching stock level for product ID: ${params.productId}`);
      response.body = await this.inventoryRepository.getStockLevel(params.productId);
      logger.debug(`Stock level for product ID: ${params.productId} fetched successfully`);
    } catch (error) {
  ...
  getAllStockLevels = async ({ response }: { response: any }) => {
    try {
  ...
  updateStockLevel = async (
    { params, request, response }: { params: { productId: string }; request: any; response: any },
  ) => {
    try {
      logger.info(`Updating stock level for product ID: ${params.productId}`);
      const { quantity } = await request.body().value;
      response.body = await this.inventoryRepository.updateStockLevel(params.productId, quantity);
      logger.debug(`Stock level for product ID: ${params.productId} updated successfully`);
    } catch (error) {
  ...
  adjustStockLevel = async (
    ...
    try {
      ...
      const { quantity } = await request.body().value;
      response.body = await this.inventoryRepository.adjustStockLevel(params.productId, quantity);
      logger.debug(`Stock level for product ID: ${params.productId} adjusted successfully`);
    } catch (error) {
  ...
  checkAvailability = async ({ params, response }: { params: { productId: string }; response: any }) => {
    try {
      logger.info(`Checking availability for product ID: ${params.productId}`);
      response.body = await this.inventoryRepository.checkAvailability(params.productId);
      logger.debug(`Availability for product ID: ${params.productId} checked successfully`);
    } catch (error) {
      logger.error("Error checking product availability: " + error.message);
      response.status = 500;
      response.body = { message: 'Erro ao verificar disponibilidade do produto' };
    }
  };

  getAllAvailability = async ({ response }: { response: any }) => {
    try {
      ...
      response.status = 500;
      response.body = { message: 'Erro ao buscar disponibilidade de todos os produtos' };
    }
  };

  setLowStockAlert = async ({ request, response }: { request: any; response: any }) => {
    try {
      logger.info("Setting low stock alert");
      const { productId, threshold } = await request.body().value;
      response.body = await this.inventoryRepository.setLowStockAlert(productId, threshold);
      logger.debug(`Low stock alert set for product ID: ${productId} with threshold: ${threshold}`);
    } catch (error) {
  ...
  getLowStockProducts = async ({ response }: { response: any }) => {
    try {
