...
describe('OrderController', () => {
  describe('GET /orders', () => {
    ...
    it('should return 500 if fetching orders fails', async () => {
      (OrderRepository.getAll as any).mockRejectedValue(new Error('Failed to fetch orders'));
      const response = await request(app).get('/orders');
  ...
  describe('POST /orders', () => {
    ...
    it('should return 500 if order creation fails', async () => {
      (OrderRepository.create as any).mockRejectedValue(new Error('Failed to create order'));
      const response = await request(app)
        .post('/orders')
  ...
  describe('GET /orders/:id', () => {
    ...
    it('should return 500 if fetching order by id fails', async () => {
      (OrderRepository.getById as any).mockRejectedValue(new Error('Failed to fetch order'));
      const response = await request(app).get('/orders/1');
      expect(response.status).toBe(500);
  ...
  describe('PUT /orders/:id/status', () => {
    ...
    it('should return 500 if updating order status fails', async () => {
      (OrderRepository.updateStatus as any).mockRejectedValue(new Error('Failed to update status'));
      const response = await request(app)
        .put('/orders/1/status')
        .send({ status: 'Shipped' });
      expect(response.status).toBe(500);
